...
class RM(schedule):
    ...
    def check_preemptive_job(self):
        try:
            # preemptive
            if (self.get_current_high_priority_job().task.period < self.current_exec_job.task.period) or \
                ((self.get_current_high_priority_job().task.period == self.current_exec_job.task.period) and\
    ...
    def get_current_high_priority_job(self):
        self.ready_queue = sorted(self.ready_queue, key= lambda x: [x.task.period, x.task.tid])
